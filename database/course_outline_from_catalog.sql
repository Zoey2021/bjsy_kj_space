-- 配套教材单元与课时（由 private_files/generate_course_outline_sql.py 根据「目录汇总.docx」生成）
-- 单元按 course_grade.name、课时按 册次+单元名 关联，避免 unit_id 序号错位导致串课
...
-- 清理孤儿课时（曾只执行 INSERT 未删旧课时时会残留错误 unit_id）
DELETE FROM course_lesson WHERE unit_id NOT IN (SELECT id FROM course_unit);

DELETE cu FROM course_unit cu
INNER JOIN course_grade cg ON cu.grade_id = cg.id WHERE cg.textbook_type = 'MAIN';

DELETE FROM course_lesson WHERE unit_id NOT IN (SELECT id FROM course_unit);

ALTER TABLE course_unit AUTO_INCREMENT = 1;
